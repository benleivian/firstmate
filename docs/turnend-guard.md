...
- Claude registers three `Stop` hooks in `.claude/settings.json`, all anchored through `CLAUDE_PROJECT_DIR`: `bin/fm-turnend-guard.sh --claude`, `bin/fm-claude-stop-autoarm.sh` with `asyncRewake: true` and `timeout: 28800`, and the daemon-scoped semantic idle event in `bin/fm-primary-busy-hook.sh`.
  The same semantic hook reports busy on `UserPromptSubmit` and idle on `StopFailure` and `SessionEnd`, but remains inert unless the away daemon has armed its private generation.
...
- Cursor registers a `stop` hook in `.cursor/hooks.json` and delegates the whole turn boundary to `bin/fm-turnend-guard-cursor.sh`, the park described below.
  ...
  The daemon-scoped primary busy hook is independently inert because a non-Claude daemon never arms its private generation.
