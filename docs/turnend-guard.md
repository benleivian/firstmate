...
- Claude registers two `Stop` hooks in `.claude/settings.json`, both anchored through `CLAUDE_PROJECT_DIR`: `bin/fm-turnend-guard.sh --claude`, and `bin/fm-claude-stop-autoarm.sh` with `asyncRewake: true` and `timeout: 28800`.
  The shared synchronous guard publishes semantic idle only after it accepts the Stop, and both synchronous and asynchronous exit-2 paths publish busy before forcing a continuation.
  `bin/fm-primary-busy-hook.sh` also reports busy on `UserPromptSubmit` and idle on `StopFailure` and `SessionEnd`, but every write remains inert unless the away daemon has armed its private generation and the event belongs to the lock-owning primary session on the native Claude host.
...
- Cursor registers a `stop` hook in `.cursor/hooks.json` and delegates the whole turn boundary to `bin/fm-turnend-guard-cursor.sh`, the park described below.
  Cursor also loads `<project>/.claude/settings.json`, so every tracked Claude-shaped continuation entrypoint whose event Cursor covers stands down on a Cursor-delivered payload through `bin/fm-hook-host-lib.sh`.
  The daemon-scoped primary busy hook is independently inert because a non-Claude daemon never arms its private generation and a foreign hook host cannot pass its primary-session identity gate.
  ...
  The guarded continuation set is the `SessionStart` entry, the two `PreToolUse` Bash entries, and the turn-end guard plus auto-arm `Stop` hooks.
...
- Grok registers a `Stop` hook in `.grok/hooks/fm-primary-turnend-guard.json` and delegates capability selection to `bin/fm-turnend-guard-grok.sh`.
  ...
  The same marker guard carries every tracked `.claude/settings.json` entry except `bin/fm-subagent-pretool-check.sh`, including the `SessionStart`, `UserPromptSubmit`, `Stop`, `StopFailure`, `SessionEnd`, and two `PreToolUse` Bash registrations.
  The subagent check is the one deliberate unguarded exception because no Grok registration covers that event, recorded in [`subagent-guard.md`](subagent-guard.md) "Known residual gap".
...
`tests/fm-turnend-guard.test.sh` covers the predicate, main and secondmate primary scope, child-worktree exclusion, `FM_HOME` and `FM_STATE_OVERRIDE` precedence, the live-lock and fresh-beacon guard predicate, the cooperative `--claude` claim wait, monotonic failed-epoch progression, bounded attended fail-open, post-alarm continuation suppression, positive recovery reset, the abandoned auto-arm claim cases that must block or clear instead of allowing a blind stop, Pi logical-run latching, missing-`jq` behavior, the tracked primary registrations, Grok native and legacy selection, typed field precedence, malformed input, and exactly-one-path safety.
