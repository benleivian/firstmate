...
A home that creates `config/turnend-churn-absorb` lets each eligible bare turn-ended task that lacks either authoritative proof use a third form: logical pane content that changed since the previous poll, compared against the same `state/.hash-*` marker the staleness backbone records, which claims no harness semantics and needs no adapter cooperation.
The watcher uses an unwrapped capture where the backend supports one and normalizes whitespace before that comparison, so a width-only rewrap neither creates a fresh stale sighting nor counts as turn-end churn.
